...
std::ostream& SoccerTeam::display() const {
    if (isEmpty()) {
        ...
        std::cout << std::left << std::setw(30) << m_teamName;
        std::cout << std::right << std::setw(15) << std::fixed << std::setprecision(2) << m_fines;
        std::cout << std::setw(6) << m_noFouls << std::setw(10);
        if (m_golas > 0) {
            std::cout << m_golas << "w" << std::endl;
        }
        else {
            std::cout << m_golas << std::endl;
        }
    }
    return std::cout;
}
